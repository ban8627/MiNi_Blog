...
const actions = {
  fetchReadMemo(context) {
    axios.get('http://banssa.dothome.co.kr/page-miniblog/read.php')
      .then(response => {
  ...
  fetchAddMemo(context, obj) {
    ...
    axios.get(`http://banssa.dothome.co.kr/page-miniblog/write.php?user=${addData.user}&title=${addData.title}&date=${addData.date}&icon=${ addData.icon }`)
      .then(response => {
  ...
  fetchDeleteMemo({
    ...
    // 아래를 실행한다.
    console.log('삭제', obj)
    axios.get(`http://banssa.dothome.co.kr/page-miniblog/delete.php?id=${obj.id}`)
      .then(res => {
  ...
  fetchUpdateMemo({
    ...
    axios.get(`http://banssa.dothome.co.kr/page-miniblog/update.php?id=${obj.item.id}&complete=${complete}`)
    .then(response => {
  ...
  fetchClearMemo({
    ...
    // 정상적으로 처리되었다면
    // 아래를 실행한다.
    axios.get(`http://banssa.dothome.co.kr/page-miniblog/delete.php?id=all`).then(res => {
...
const mutations = {
  ...
  ADD_MEMO(state, payload) {
    ...
    // 가지고 와서 목록 1개를 추가한다.
    axios.get(`http://banssa.dothome.co.kr/page-miniblog/get.php?user=${payload.user}`)
      .then(res => {
        // console.log('입력완료', res.data.result)
